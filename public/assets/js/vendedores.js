// PIN_ACESSO e SHEETS_URL vêm de assets/js/config.js

// ── SESSÃO ────────────────────────────────────────────────────────────────────
let SESSION = null; // { email, senha, nome }

function salvarSession(email, senha, nome) {
  SESSION = { email, senha, nome };
  sessionStorage.setItem('pf_vendedora', JSON.stringify(SESSION));
}
function carregarSession() {
  try {
    const s = sessionStorage.getItem('pf_vendedora');
...
function logout() {
  SESSION = null;
  sessionStorage.removeItem('pf_vendedora');
...
function mostrarHistorico(data) {
  ...
  document.getElementById('hist-stats').innerHTML = `
    <div class="stat-card"><div class="stat-label">Pedidos Realizados</div><div class="stat-value purple">${data.qtd_pedidos}</div></div>
...
function renderMeusCupons(cupons) {
  ...
  lista.innerHTML = cupons.map((c, idx) => {
    ...
    if (c.produtos === 'todos') {
      ...
      const linhaStyle = 'font-size:.72rem;color:var(--gray);line-height:1.8';
      prodsHtml = `<div style="${linhaStyle};margin-top:5px">
        ...
        ${rest.length ? `
          ...
          <button id="pb-${idx}" onclick="toggleProdsList(${idx})"
            style="background:transparent;border:none;color:var(--lavender);font-size:.7rem;cursor:pointer;padding:3px 0;font-family:inherit;text-decoration:underline;text-underline-offset:2px">
    ...
    return `<div class="sec" style="margin-bottom:14px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
        ...
        <button onclick="deletarCupomMeu('${c.codigo}',this)"
          style="background:transparent;border:1px solid var(--red);color:var(--red);padding:8px 16px;border-radius:8px;font-size:.78rem;font-weight:700;cursor:pointer;white-space:nowrap;flex-shrink:0">
          🗑️ Excluir
        </button>
      </div>
    </div>`;
