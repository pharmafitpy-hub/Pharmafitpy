...
function calcularDescontoCupom() {
  ...
  if (cupomData.tipo === 'combo') {
    ...
    // Conta qty de cada item do combo no carrinho
    const qtyPorItem = {};
    itensCombo.forEach(id => qtyPorItem[id] = 0);
    Object.entries(cart).forEach(([key, qty]) => {
      if (cupomData.modo === 'variante') {
    ...
    // Preço normal de UM combo (1 unidade de cada item)
    let precoNormalUmCombo = 0;
    itensCombo.forEach(id => {
      // modo='variante': id = "prod__idx". modo='qualquer': id = baseId
      const preco = getPriceByKey(id) || 0;
      precoNormalUmCombo += preco;
    });
...
function renderProducts() {
  ...
  list.forEach(p => {
    ...
    grid.innerHTML += `
      <div class="product-card ${cardSelected ? 'selected' : ''} ${promoAtiva ? 'promo-ativa' : ''} ${(p.imagem || p.foto) ? 'pc-has-image' : 'pc-no-image'}"
