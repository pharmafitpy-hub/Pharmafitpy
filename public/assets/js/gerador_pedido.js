...
function aplicarCupom(){
  ...
  if(c!==undefined){
    ...
    const extras=[];
    if(cupomData.parcelamento)extras.push('⚡ Parcelamento sem juros em até 3×');
    if(cupomData.frete_gratis_acima>0)extras.push(`🚚 Frete grátis acima de R$ ${cupomData.frete_gratis_acima.toLocaleString('pt-BR',{minimumFractionDigits:2})}`);
    let txt='';
    ...
    if(extras.length)txt+=`<br><small style="color:var(--green)">${extras.join(' · ')}</small>`;
    ...
    if(cupomData.parcelamento){
      ...
      const sel=document.getElementById('c_parcelas');
    ...
    if(cupomData.frete_gratis_acima>0&&calcSubtotal()>=cupomData.frete_gratis_acima){
      freteValor=0;freteMetodo='gratis-cupom';
      const gratisEl=document.getElementById('frete-gratis');
      gratisEl.textContent=`🎁 Frete grátis pelo cupom ${codigo}!`;
...
function removerCupom(){
  ...
  // Restaurar parcelas normais
  const sel=document.getElementById('c_parcelas');
  if(sel.dataset.cupomParc){delete sel.dataset.cupomParc;renderParcelasSelect();}
  onPagamentoChange();
  // Restaurar frete
  if(freteMetodo==='gratis-cupom'){freteValor=0;freteMetodo='';document.getElementById('frete-gratis').style.display='none';}
  renderCart();gerarMensagem();
}
...
function getParcelaInfo(){
  ...
  return{parcelas:parseInt(sel.value)||1,juros:parseFloat(opt.getAttribute('data-juros'))||0};
}
...
function renderCart(){
  ...
  list.innerHTML=keys.map(key=>{
    ...
    const qty=cart[key];const price=getItemPrice(key);let conc=p.conc;
  ...
  let totalFinal=subtotal+freteValor-desconto;
  if(parcInfo&&parcInfo.parcelas>1&&parcInfo.juros>0){
    totalFinal=(subtotal+freteValor)*(1+parcInfo.juros/100);const vp=totalFinal/parcInfo.parcelas;
...
function gerarMensagem(){
  ...
  keys.forEach(key=>{
    ...
    const qty=cart[key];const price=getItemPrice(key);let conc=String(p.conc);if(vi!==null&&p.variantes[vi])conc=p.variantes[vi].dose;
    const sub=price*qty;subtotal+=sub;
  ...
  // Cupom
  const desconto=calcularDescontoCupom();
  if(desconto>0)msg+=`🎟️ Cupom ${cupomCodigo}: − R$ ${desconto.toLocaleString('pt-BR',{minimumFractionDigits:2})}\n`;
  // Parcelas
  const pi=getParcelaInfo();let total=subtotal+freteValor-desconto;
  if(pi&&pi.parcelas>1&&pi.juros>0){total=(subtotal+freteValor)*(1+pi.juros/100);const vp=total/pi.parcelas;msg+=`💰 *SUBTOTAL: R$ ${subtotal.toLocaleString('pt-BR',{minimumFractionDigits:2})}*\n💳 *${pi.parcelas}x de R$ ${vp.toLocaleString('pt-BR',{minimumFractionDigits:2})}* (${pi.juros}% juros)\n💰 *TOTAL: R$ ${total.toLocaleString('pt-BR',{minimumFractionDigits:2})}*\n`;}
...
function limparPedido(){
  if(!confirm('Limpar o pedido atual?'))return;cart={};freteValor=0;freteMetodo='';removerCupom();
